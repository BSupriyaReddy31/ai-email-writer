...
import sqlite3
import os
import urllib.parse # --- NEW: Required to format the email link safely ---
        ...
        if generate_btn:
            if not recipient or not context:
                ...
                with st.spinner("🧠 AI is crafting your email..."):
                    ...
                    st.text_area("Review and Copy:", value=draft, height=350, label_visibility="collapsed")
                    
                    # --- NEW: Mailto Button Logic ---
                    st.divider()
                    if target_email:
                        # Create a safe subject line and body for the URL
